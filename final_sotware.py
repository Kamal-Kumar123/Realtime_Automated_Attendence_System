...
def resolve_facenet_model_path(user_path):
    """Turn UI input into a path for facenet.load_model (.pb file or checkpoint directory)."""
    default_rel = os.path.join("20180402-114759", "20180402-114759.pb")
    ...
    if not os.path.isdir(p):
        raise FileNotFoundError("FaceNet model path does not exist: %s" % user_path)
    pb_here = os.path.join(p, "20180402-114759.pb")
    if os.path.isfile(pb_here):
        return pb_here
    nested_pb = os.path.join(p, "20180402-114759", "20180402-114759.pb")
    if os.path.isfile(nested_pb):
        return nested_pb
    try:
        names = os.listdir(p)
    except OSError as e:
        raise FileNotFoundError("Cannot read directory %r: %s" % (p, e)) from e
    if any(n.endswith(".meta") for n in names):
        return p
    nested_dir = os.path.join(p, "20180402-114759")
    if os.path.isdir(nested_dir):
        try:
            n2 = os.listdir(nested_dir)
        except OSError:
            n2 = []
        if any(n.endswith(".meta") for n in n2):
            return nested_dir
    raise FileNotFoundError(
        "No FaceNet model under %r (need 20180402-114759.pb or a folder with .meta/.ckpt)."
        % p
    )
    ...
    if path and os.path.isdir(path):
        ...
        if os.path.isdir(path):
            ...
            try:
                ...
                sys.exit()
    print("Dataset images will be saved under:", os.path.abspath(path))
    res = webcam
    if res == "":
        res = (640, 480)
    else:
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            while True:
                ...
                personNo += 1
                users_folder = os.path.join(path, folder_name)
                image_no = 1
                ...
                print(
                    "OpenCV window: focus it, show your face, then press S to save a crop, Q to finish this person."
                )
                while loop_type or (total_frames > 0):
                    ...
                    if not ret or image is None:
                        print("Could not read frame from camera/video.")
                        break
                    ...
                    if key == ord("s") or key == ord("S"):
                        bb, _pts = detect_face.detect_face(
                            image, minsize, pnet, rnet, onet, threshold, factor
                        )
                        has_face = (
                            bb is not None
                            and getattr(bb, "ndim", 0) >= 2
                            and bb.shape[0] > 0
                        )
                        if has_face:
                            x1, y1, x2, y2 = (
                            ...
                            image_name = os.path.join(
                                users_folder,
                                folder_name + "_" + str(image_no).zfill(4) + ".png",
                            )
                            if cv2.imwrite(image_name, aligned_image):
                                print("Saved:", os.path.abspath(image_name))
                                image_no += 1
                            else:
                                print("Failed to write image:", image_name)
                        else:
                            print("No face detected — center your face and try S again.")
...
def train(parameters):
    ...
    if not path:
        ...
        path = os.path.normpath(os.path.expanduser(path1))
    ...
    model = resolve_facenet_model_path(path2)
    ...
    if not os.path.isdir(path):
        raise FileNotFoundError(
            "Dataset folder not found: %r (need a directory with one subfolder per person)."
            % path
        )

    dataset = facenet.get_dataset(path)
    dataset = [
        facenet.ImageClass(c.name, _face_image_paths(c.image_paths)) for c in dataset
    ]
    dataset = [c for c in dataset if len(c.image_paths) > 0]
    if len(dataset) == 0:
        raise ValueError(
            "No image files under %r. Add .png/.jpg face crops in subfolders (e.g. output/kamal/*.png)."
    ...
    if split_dataset == 'y':
        for cls in dataset:
            ...
            n = len(paths)
            # Old formula int(pct * n * 0.01) is 0 for small n — yields empty train set.
            if n == 1:
                train_set.append(facenet.ImageClass(cls.name, paths))
                test_set.append(facenet.ImageClass(cls.name, []))
            else:
                no_train = int(round(percentage * n / 100.0))
                no_train = max(1, no_train)
                no_train = min(no_train, n - 1)
                train_set.append(facenet.ImageClass(cls.name, paths[:no_train]))
                test_set.append(facenet.ImageClass(cls.name, paths[no_train:]))
    ...
    if len(paths_train) == 0:
        raise ValueError(
            "No images in the training set after split. Use more photos per person or disable train/test split."
        )
    ...
    if split_dataset == 'y' and len(paths_test) > 0:
    ...
    elif split_dataset == 'y':
        print("\nSkipping test accuracy (no held-out images — add more photos per person or use fewer for train).")
